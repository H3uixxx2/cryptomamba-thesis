"""Predict screen API — reuses api_client (live) + predict_artifacts (offline).
...
are flagged. No model inference happens here — live calls the remote Colab API,
offline reads the frozen artifact.
"""
...
def _iso_date(value: str, *, field: str) -> date:
    try:
        ...
        raise HTTPException(
            status_code=400,
            detail=f"{field} must use valid YYYY-MM-DD format.",
        ) from exc
    if parsed.isoformat() != value:
        raise HTTPException(
            status_code=400,
            detail=f"{field} must use YYYY-MM-DD format.",
        )
    return parsed
...
def _checkpoint_window_rows(
    ...
    if any(
        ...
        raise HTTPException(
            status_code=400,
            detail="Candle dates must be unique and strictly increasing.",
        )
    ...
    if any(
        ...
        raise HTTPException(
            status_code=400,
            detail="Selected candle dates must be contiguous daily.",
        )
...
def predict_checkpoint(req: CheckpointPredictionRequest) -> dict:
    ...
    try:
        ...
        raise HTTPException(
            status_code=503,
            detail=f"Verified checkpoint evidence is unavailable: {exc}",
        ) from exc
    ...
    if (
        ...
        raise HTTPException(
            status_code=500,
            detail="Checkpoint worker window metadata does not reconcile with the request.",
        )
...
def predict_live(req: LivePredictRequest) -> dict:
    ...
    if "predicted_close" not in prediction:
        raise HTTPException(
            status_code=502,
            detail="Model API response is missing 'predicted_close'.",
        )
