...
Two clearly separated capabilities (Phase 4 spec):
  * ``POST /simulate`` — the one-day decision demo (reuses trading_logic.simulate_trade).
  * ``GET  /backtest`` — the thesis-grade historical chronological backtest, read from the
    artifacts produced offline by CryptoMamba/scripts/run_backtest.py
    (trading_metrics.csv / trading_equity_curve.csv / regime_metrics.csv + metadata). No
    model, no recompute here — pure artifact presentation. Missing artifact => NOT_READY.
"""
...
def simulate(req: SimulateRequest) -> dict:
    realized_price = req.current * (1 + req.realized_move / 100.0)
    sim = logic.trading_logic.simulate_trade(
        req.current, req.predicted, req.capital, req.btc, req.risk, realized_price
    )
    rows = []
    ...
    return {
        "start_value": req.capital + req.btc * req.current,
        "assumed_close": realized_price,
        "move_pct": logic.trading_logic.pct(req.predicted, req.current),
        "rows": rows,
...
def replay(
    ...
    if (
        ...
        raise HTTPException(
            status_code=400,
            detail=f"Unsupported replay selection. Available values: {available}",
        )
