...
Vietnamese seven-chapter thesis with English UI/chart/diagram labels. The document separates:
...
XeLaTeX is required because the source uses `fontspec` and Vietnamese `polyglossia`.

```bash
cd /Users/hieutha/PycharmProject/thesis_draft/latex
tectonic -X compile main.tex --keep-logs
```

## Upload to Overleaf

Upload `final/cryptomamba_thesis_overleaf.zip` as a new project and select **XeLaTeX**. The delivery
